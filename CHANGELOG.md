...
- **数据库损坏自愈**（`database/goal_db.py`）：加载时若 goals.db 文件损坏
  （`database disk image is malformed` / `not a database` 等
  `sqlite3.DatabaseError`，常见诱因为强杀进程、同步盘/杀毒软件干扰），
  不再导致插件 on_load 失败——自动关闭连接、把损坏的库文件与
  -wal/-shm 侧车按时间戳改名隔离（保留为 `*.corrupt-<时间戳>` 供排查，
