> 🗄️ **归档文档**（2026-09-02 归档）
>
> 本文件是 **v4.5.0 及更早版本**的 README 原文归档，仅作历史参考，**不再随版本更新**。
> 其中描述的工具列表、命令列表与配置项已过时——v4.6.0 移除了
> `generate_schedule_v4` 工具与 `/plan status` 命令，作息配置更名为
> `wake_time`（起床）/ `sleep_time`（入睡），并精简了部分配置项。
> 当前版本的说明请看 **[README.md](README.md)**，版本变更详见
...
> 回复时贴合生活节奏。本页前半部分是 v4.6.0 变更摘要与 v4.5.0 说明；**后半部分为原版文档，
> 内容逐字保留**（其中工具/命令/配置描述以 v4.6.0 变更摘要为准）。
...
## v4.6.0 变更摘要（2026-09-02）

> ⚠️ 本版本含破坏性变更，旧配置会自动迁移，详见 [CHANGELOG.md](CHANGELOG.md)。

- **作息语义重构**：配置项改为 **起床（睡醒）时间 `wake_time`** 与 **入睡时间 `sleep_time`**，
  清醒活动排在两者之间，入睡到次日起床为睡眠时段（原 `day_start_time/day_end_time` 自动迁移）。
- **无睡眠模式全面兼容**：提示词示例与时间框架按作息动态生成，不再自相矛盾；
  睡眠时段在无睡眠模式下排"无所事事"。
- **移除** `generate_schedule_v4` 工具与 `/plan status` 命令；**`/plan list`** 现在
  合并转发"图片 + 详细文字"两条记录，今日无日程时自动先生成再展示；
  图片绘制超时（`image_timeout_seconds`）或关闭绘制（`list_draw_image`）时静默降级为纯文字。
- **配置精简**：移除 `auto_generate` / `inject_mode` / 次日推断微调字段；
  角色裁判配置补充完整机制说明。
- **死代码清理**：删除全部无调用的模块 / 方法 / 异常类 / 常量（详见 CHANGELOG）。
